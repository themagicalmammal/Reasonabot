...
def send_welcome(message):
    bot.reply_to(
        message,
        "My superbot ability is Searching lowest prices for any mobile phone you want",
    )
    bot.reply_to(
        message, "But I can tell you a joke if you insist( Type /tellmeajoke)")
...
def send_welcome(message):
    bot.reply_to(
        message,
        "Thor ask ultron whats your age. Ultron told he doesnt know. Told u age of ultron doesnt make sense....",
    )
...
def send_welcome(message):
    ...
    bot.reply_to(message,
                 "Ashish Shah @alex55936 and Dipan Nanda @DarkDevil1999")
...
def send_welcome(message):
    bot.reply_to(
        message,
        "I feed on dynamic and static web pages and use web scraping to digest it",
    )
...
def send_welcome(message):
    bot.reply_to(
        message,
        "I was built using dynamic and static webscrapping using selenium and beautifulsoup on pythontelgrambot",
    )
    ...
    try:
        ...
        for i in productnames:
            if j < 5:
                ...
                j += 1
        daz = bot.reply_to(
            message, "Which one are you looking for?(Only type the Index)")
...
def process_next_step(message):
    try:
        ...
        storage = soup.findAll("li",
                               {"class": "kyspc__item kyspc__item--strge"})
        battery = soup.findAll("li",
                               {"class": "kyspc__item kyspc__item--bttry"})
        camera = soup.findAll("li", {"class": "kyspc__item kyspc__item--cmra"})
        screensize = soup.findAll("li",
                                  {"class": "kyspc__item kyspc__item--aspct"})
